...
impl Factory {
    ...
    /// Call once after deploy: registers the wallet byte-code hash.
    ...
    /// Deploy a new wallet and initialise it with the user's passkey.
    ///
    /// * `salt` – 32-byte salt for the contract
    ...
    pub fn deploy(env: Env, salt: BytesN<32>, pk: BytesN<65>) -> Result<Address, Error> {
        ...
        #[allow(deprecated)]
        let address = deployer.deploy(wasm_hash);
